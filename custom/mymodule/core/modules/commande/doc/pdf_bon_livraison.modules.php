 ...
 */
class pdf_bon_livraison extends ModelePDFCommandes
{
	/**
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $titlekey = "PdfOrderTitle")
{
    ...
    $hautcadre = 40; // Define the height of the recipient frame
    $pdf->SetTextColor(0, 0, 0);
    $pdf->SetFont('', '', $default_font_size - 2);
...
$this->cols['desc'] = array(
    ...
    'status' => true,
    'title' => array(
        'textkey' => 'ProductName', // Change to 'Reference' or 'ProductName'
    ),
    'content' => array(
        'align' => 'L',
        'padding' => array(1, 0.5, 1, 1.5),
    ...
    foreach ($object->lines as $line) {
        if (!empty($line->fk_product)) {
            $sql = "SELECT COUNT(*) as nb FROM ".MAIN_DB_PREFIX."product_lot WHERE fk_product = ".((int) $line->fk_product);
